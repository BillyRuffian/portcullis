  ...
  class DivisionGroupedByParty
    ...
    def self.openapi_types
      {
        ...
        :'aye_count' => :'Integer',
        :'no_count' => :'Integer',
        :'ayes' => :'Array<PartyVoteResult>',
        :'noes' => :'Array<PartyVoteResult>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ParliamentCommonsVotes::DivisionGroupedByParty` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ParliamentCommonsVotes::DivisionGroupedByParty`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [division_id, number, title, date, aye_count, no_count, ayes, noes].hash
    end
